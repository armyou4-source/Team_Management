...
export const INTERVIEW_PERIOD_TYPES = ['중간면담', '성과평가', '수시면담'] as const;
export type InterviewPeriodType = (typeof INTERVIEW_PERIOD_TYPES)[number];
...
export interface InterviewPeriodMeta {
  periodKey: string;
  label: string;
  year: number;
  purposeType: InterviewPeriodType;
  isActive: boolean;
  startedAt: string;
...
interface TeamInterviewPeriodRow {
  period_key: string;
  label: string;
  year: number;
  purpose_type: InterviewPeriodType;
  is_active: boolean;
  started_at: string;
...
export const buildPeriodKey = (year: number, periodType: InterviewPeriodType): string =>
  `${year}-${periodType}`;

export const buildPeriodLabel = (year: number, periodType: InterviewPeriodType): string =>
  `${year}년 ${periodType}`;

export const formatInterviewPeriodLabel = (periodKey: string, fallbackLabel?: string): string => {
  if (fallbackLabel) return fallbackLabel;
  if (periodKey === LEGACY_INTERVIEW_PERIOD_KEY) return '이전 기록';
  const match = periodKey.match(/^(\d{4})-(.+)$/);
  if (!match) return periodKey;
  return `${match[1]}년 ${match[2]}`;
};

export const mapPeriodRowToMeta = (row: TeamInterviewPeriodRow): InterviewPeriodMeta => ({
...
export const mapRowToForm = (
  ...
  return {
    date: row.면담일자 ?? '',
    purpose: row.면담목적 ? normalizeInterviewPurpose(row.면담목적) : '',
    content: row.주요면담내용 ?? '',
    feedback: row.피드백조치 ?? '',
    complaints,
...
export const mapHistoryRowToEntry = (row: TeamInterviewHistoryRow): InterviewHistoryEntry => ({
  ...
  source: 'history',
  periodKey: row.interview_period ?? undefined,
});
...
export const buildInterviewPayload = (
  ...
  건의상태: form.complaints.trim()
    ...
    : null,
  상태: status,
  interview_period: interviewPeriod,
});

export const isMissingPeriodTableError = (error: { code?: string; message?: string }): boolean =>
  error.code === 'PGRST205' || (error.message?.includes('team_interview_periods') ?? false);

export const fetchInterviewPeriods = async (): Promise<InterviewPeriodMeta[]> => {
  const { data, error } = await supabase
    .from('team_interview_periods')
    .select('*')
    .order('started_at', { ascending: false });

  if (error) {
    if (isMissingPeriodTableError(error)) {
      return [
        {
          periodKey: DEFAULT_INTERVIEW_PERIOD_KEY,
          label: buildPeriodLabel(2026, '중간면담'),
          year: 2026,
          purposeType: '중간면담',
          isActive: true,
...
export const ensureActiveInterviewPeriod = async (): Promise<InterviewPeriodMeta> => {
  ...
  const purposeType: InterviewPeriodType = '중간면담';
  const periodKey = buildPeriodKey(year, purposeType);
  const label = buildPeriodLabel(year, purposeType);

  const { error } = await supabase.from('team_interview_periods').insert({
  ...
  if (error && !isMissingPeriodTableError(error)) {
    throw error;
  }
...
export const fetchInterviewsForPeriod = async (
  periodKey: string
): Promise<Record<string, InterviewRecord>> => {
  const { data, error } = await supabase
    .from('team_interview')
    .select('*')
    .eq('interview_period', periodKey);
...
export const fetchInterviewsFromSupabase = async (): Promise<Record<string, InterviewRecord>> => {
  const activePeriod = await ensureActiveInterviewPeriod();
  return fetchInterviewsForPeriod(activePeriod.periodKey);
};
...
export const saveInterviewToSupabase = async (
  ...
  const payload = buildInterviewPayload(emp, form, status, interviewPeriod);
  const { error } = await supabase
    .from('team_interview')
    .upsert({ ...payload, updated_at: new Date().toISOString() }, {
      onConflict: '사번,interview_period',
    });
...
export const fetchInterviewHistoryForEmployee = async (
  ...
  if (interviewPeriod) {
    query = query.eq('interview_period', interviewPeriod);
  }
...
export const fetchLoadableInterviewEntriesForEmployee = async (
  emp: EmployeeRef,
  periodKey: string,
  periodLabel: string
): Promise<InterviewHistoryEntry[]> => {
  const historyEntries = (await fetchInterviewHistoryForEmployee(emp, periodKey)).map((entry) => ({
    ...entry,
    periodKey: entry.periodKey ?? periodKey,
    periodLabel,
  }));
  ...
  const row = data as TeamInterviewRow;
  const form = mapRowToForm(row);
  const status = normalizeInterviewStatus(row.상태);

  if (!hasInterviewContent(form) && status === '미면담') {
    return [];
  }
...
export const createEmptyPeriodInterviewForm = (): InterviewForm => ({
  date: '',
  purpose: '',
  content: '',
  feedback: '',
...
export const startNewInterviewPeriod = async (
  employees: EmployeeRef[],
  year: number,
  purposeType: InterviewPeriodType
): Promise<InterviewPeriodMeta> => {
  const periodKey = buildPeriodKey(year, purposeType);
  const label = buildPeriodLabel(year, purposeType);
  ...
  if (existingError && !isMissingPeriodTableError(existingError)) {
    throw existingError;
  }
  if (existingPeriod) {
    throw new Error(`${label} 시기가 이미 존재합니다.`);
  }
  ...
  if (deactivateError && !isMissingPeriodTableError(deactivateError)) {
    throw deactivateError;
  }

  const { error: insertPeriodError } = await supabase.from('team_interview_periods').insert({
    period_key: periodKey,
    label,
    year,
    purpose_type: purposeType,
  ...
  if (insertPeriodError && !isMissingPeriodTableError(insertPeriodError)) {
    throw insertPeriodError;
  }

  const emptyForm = createEmptyPeriodInterviewForm();
  const rows = employees.map((emp) => ({
    ...buildInterviewPayload(emp, emptyForm, '미면담', periodKey),
  ...
  if (rows.length > 0) {
    const { error: insertRowsError } = await supabase
      .from('team_interview')
      .upsert(rows, { onConflict: '사번,interview_period' });
